...
@Disabled
@TeleOp(name ="Very important on-topic op mode aka Tetris", group = "Tetris")
public class Tetris extends LinearOpMode {
    ...
    boolean lines[][] = new boolean[10][8];  //a 2d array that is an 20x10 "screen", think of each coordinate (example: lines[0][0]) as a pixel.  if the coordinate value is true then █ displays, but if the value is false then ░ displays.
    ...
    public void runOpMode() { //before reading anything under runOpMode(), read all the methods that are called during this method first
        ...
        */
        ...
        telemetry.log().add("Tetris by 6547 Cobalt Colts & adapted by 16265");
        telemetry.log().add("");
        ...
        while (!isStarted())
        {
            if (gamepad1.y && gamepad1.b) { //if y or b is pressed, a
                debugMode = !debugMode;
                telemetry.log().add((debugMode) ? "Debug mode enabled, hey dhgar man fam" : "Debug mode disabled");
        ...
        while (opModeIsActive()) {
            ...
            while (opModeIsActive() && pieceState!=touchedFor1Second)
            {
                ...
                if (gamepad1.x && !gamepad1.xWasPressed()) rotatePiece(); //changed dis
    ...
    public void useGamepadInput()  //this is a method so that way it's easier to comment out in runOpMode() for debugging purposes
    {
        ...
        if (gamepad1.dpad_down && !gamepad1.dpadDownWasPressed())
        {
            if (debugMode)
            {
